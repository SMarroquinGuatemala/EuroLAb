    ...
    public partial class FrmClienteFiltro : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {           
                ...
                grdCliente.DataSource = cliente.GetAll(DepartamentoID, MunicipioID);
                lblRegsitrosProducto.Text = grdCliente.MainView.RowCount.ToString();
            }
            catch (Exception ex)
            {
                Log.Add(ex);
            ...
            try
            {
                if (e.Button == MouseButtons.Right)
                {
                    ...
                    if (ViewPrincipal.DataRowCount > 1)
                    {
                        ...
                        mnuPrecioProducto.Enabled = true;
                    }
                }
            }
            catch (Exception ex)
        ...
        private void precioClienteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            object ClienteIDValue = 0;
            ClienteIDValue = ViewPrincipal.GetRowCellValue(ViewPrincipal.FocusedRowHandle, ClienteID);
            Form form = new FrmPrecioClienteProducto((int)ClienteIDValue,0);
            form.ShowDialog();
        }
